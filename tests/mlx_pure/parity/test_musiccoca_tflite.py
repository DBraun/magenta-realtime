...
"""Parity: mlx_pure MusicCoCa vs the original TFLite models.
...
def module():
    from magenta_rt.mlx_pure.musiccoca import from_safetensors
...
def test_frontend_parity(module, waveform):
    ...
    got = np.array(module.frontend(mx.array(waveform[None])))
...
def test_audio_embed_end_to_end(module, waveform):
    ...
    got = np.array(module.embed_audio(mx.array(waveform[None])))
...
def test_text_encoder_parity(module, vocab, text):
    ...
    from magenta_rt.mlx_pure.musiccoca import encode_text

    ids, paddings = encode_text(vocab, text)
    ref = _run_tflite("text_encoder", ids[None], paddings[None])
    got = np.array(module.embed_text(mx.array(ids[None]), mx.array(paddings[None])))
    np.testing.assert_allclose(got, ref, atol=1e-4)
...
def test_quantizer_parity(module, vocab):
    ...
    got = np.array(module.tokenize(mx.array(emb))).flatten()
...
def test_mapper_parity(module, vocab):
    ...
    got = np.array(module.mapper(mx.array(emb), mx.array(noise[None])))
    np.testing.assert_allclose(got, ref, atol=1e-4)


def test_system_matches_tflite_system(vocab):
    """End-to-end: mlx_pure MusicCoCa class vs the TFLite-backed MusicCoCa."""
    from magenta_rt import musiccoca as tflite_musiccoca
    from magenta_rt.mlx_pure import musiccoca as mlx_musiccoca

    ref_model = tflite_musiccoca.MusicCoCa(resource_dir=_RESOURCE_DIR)
    got_model = mlx_musiccoca.MusicCoCa(resource_dir=_RESOURCE_DIR)
