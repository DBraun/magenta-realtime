...
"""Parity: functional jax MusicCoCa vs the original TFLite models.
...
def params():
    ...
    return mc.load_params(_RESOURCE_DIR / "musiccoca_nnx.safetensors")
...
def test_frontend_parity(params, waveform):
    from magenta_rt.jax import musiccoca as mc

    ref = _run_tflite("audio_preprocessor", waveform[None])
    got = np.asarray(mc.log_mel_spectrogram(params["frontend"], waveform[None]))
    np.testing.assert_allclose(got, ref, atol=1e-4)


def test_audio_embed_end_to_end(params, waveform):
    from magenta_rt.jax import musiccoca as mc

    mel = _run_tflite("audio_preprocessor", waveform[None])
    ref = _run_tflite("music_encoder", mel)
    got = np.asarray(mc.embed_audio(params, waveform[None]))
...
def test_text_encoder_parity(params, vocab, text):
    from magenta_rt.jax import musiccoca as mc

    ids, paddings = mc.encode_text(vocab, text)
    ref = _run_tflite("text_encoder", ids[None], paddings[None])
    got = np.asarray(mc.embed_text(params, ids[None], paddings[None]))
    np.testing.assert_allclose(got, ref, atol=1e-4)


def test_quantizer_parity(params, vocab):
    from magenta_rt.jax import musiccoca as mc

    ids, paddings = mc.encode_text(vocab, "ambient piano")
    emb = _run_tflite("text_encoder", ids[None], paddings[None])
    ref = _run_tflite("pretrained_vector_quantizer", emb).flatten()
    got = np.asarray(mc.tokenize(params, emb)).flatten()
    np.testing.assert_array_equal(got, ref)


def test_mapper_parity(params, vocab):
    from magenta_rt.jax import musiccoca as mc
    ...
    got = np.asarray(mc.map_text_embedding(params, emb, noise[None]))
    np.testing.assert_allclose(got, ref, atol=1e-4)


def test_system_matches_tflite_system(vocab):
    """End-to-end: jax MusicCoCa class vs the TFLite-backed MusicCoCa."""
    from magenta_rt import musiccoca as tflite_musiccoca
    from magenta_rt.jax import musiccoca as jax_musiccoca

    ref_model = tflite_musiccoca.MusicCoCa(resource_dir=_RESOURCE_DIR)
    got_model = jax_musiccoca.MusicCoCa(resource_dir=_RESOURCE_DIR)
