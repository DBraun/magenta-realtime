...
This subpackage represents the original production codebase target, modeling modules as explicit stateful sequence combinators (`sl.SequenceLayer`, `sl.Emitting`) threaded with runtime latency tracking buffers. For a zero-dependency sibling implementation using plain `mlx.nn.Module` classes, see [`magenta_rt.mlx_pure`](../mlx_pure/README.md).
